...
interface Props {
  cat: DisplayCategory;
  onSave: (updated: DisplayCategory) => void;
  onClose: () => void;
}

const presetColors = ['#FF5722', '#4CAF50', '#2196F3', '#FFC107', '#9C27B0', '#00BCD4', '#795548', '#607D8B'];

export const CategoryEditModal = ({ cat, onClose, onSave }: Props) => {
  const [icons, setIcons] = useState<AppIcon[]>([]);
  const [name, setName] = useState(cat.name);
  const [selectedIcon, setSelectedIcon] = useState<keyof typeof MaterialCommunityIcons.glyphMap>(cat.icon);
  const [selectedIconId, setSelectedIconId] = useState<number>(cat.iconId);
  const [selectedColor, setSelectedColor] = useState(cat.color);
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.iconScroll}>
            {icons.map(icon => (
              <TouchableOpacity
                ...
                style={[
                  styles.iconBox,
                  selectedIcon === icon.name && styles.selectedIconBox,
                ]}
                onPress={() => {
